...
class PrioritizedReplayBuffer:
    def __init__(self, size, alpha, device="cuda"):
        """
    ...
    def add(self, obs_t, action, obs_tp1, reward, done):
        """
    ...
    def sample(
        ...
        state, actions, next_state, reward, dones = [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
            obs_t, action, rew, obs_tp1, done = data
            state.append(np.array(obs_t, copy=False))
            actions.append(np.array(action, copy=False))
            reward.append(rew)
            next_state.append(np.array(obs_tp1, copy=False))
            dones.append(done)
        return (
            weights,
            idxes,
